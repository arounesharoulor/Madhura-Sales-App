...
export default function MeetingScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!/^\d{10}$/.test(phone.trim())) {
      Alert.alert('Invalid Phone', 'Contact Phone must be exactly 10 digits.');
      return;
    }
    if (!locationCoords && Platform.OS !== 'web' && meetingType !== 'Online') {
      Alert.alert('GPS', 'Waiting for GPS coordinates. Please wait.');
      return;
    }

    if (meetingType === 'Online' && !form.onlineMeetingLink) {
      Alert.alert('Validation', 'Online meeting link is required.');
      return;
    }
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
          ...
          >
            ...
            {[
              ...
              { key: 'phone', label: 'Contact Phone *', icon: 'call-outline', placeholder: '10-digit number', keyboard: 'phone-pad', maxLength: 10 },
            ].map(f => (
