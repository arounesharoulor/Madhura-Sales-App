      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, justifyContent: 'space-between', marginBottom: 20 }}>
            {(isHR || isMD) && (
              <StatCard label="Checked In Today" value={metrics.checkedIn} icon="log-in" color="#10b981" onPress={() => navigation.navigate('AdminAttendance')} />
            )}
            {(isHR || isMD) && (
              <StatCard label="Checked Out Today" value={metrics.checkedOut} icon="log-out" color="#6366f1" onPress={() => navigation.navigate('AdminAttendance')} />
            )}
            {!isHR && (
              <StatCard label="Total Staff" value={metrics.totalEmployees} icon="people" color="#6366f1" onPress={() => navigation.navigate('UserManagement')} />
            )}
            {!isHR && (
              <StatCard label="Open Tasks" value={metrics.openTasks} icon="clipboard" color="#f43f5e" onPress={() => navigation.navigate('TaskAssignment')} />
            )}
            <StatCard label="Total Clients" value={metrics.totalClients} icon="briefcase" color="#7c3aed" onPress={() => navigation.navigate('ClientOnboarding')} />
            <StatCard label="Visits Today" value={metrics.clientVisitsToday} icon="location" color="#0ea5e9" onPress={() => navigation.navigate('Meeting')} />
          </View>
